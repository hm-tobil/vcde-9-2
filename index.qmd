...
<!-- =====================================================================
     ...
     ===================================================================== -->
<style>
  /* Gemeinsame Flat-UI-Palette für die ganze Seite (Bootswatch "flatly"
     verwendet dieselben Farben, dadurch wirken native Elemente wie
     Überschriften, Links und Tabellen konsistent zu den Slidern). */
  :root {
    --vslam-navy: #2c3e50;
    --vslam-blue: #3498db;
    --vslam-blue-dark: #2980b9;
  }

  h1, h2, h3 { color: var(--vslam-navy); }
  h2 {
    border-bottom: 3px solid var(--vslam-blue);
    padding-bottom: .35rem;
    margin-top: 2.75rem;
  }
  a { color: var(--vslam-blue-dark); }
  a:hover { color: var(--vslam-navy); }
  ...
  .vslam-slider .vslam-slide h4 {
    margin-top: 0;
    color: var(--vslam-navy);
    border-bottom: 2px solid var(--vslam-blue);
    padding-bottom: .35rem;
  }
  ...
  .vslam-slider .vslam-btn {
    background: var(--vslam-navy); color: #fff; border: none;
    border-radius: 6px; padding: .35rem .9rem;
    cursor: pointer; font-size: .9rem;
    transition: background .15s ease;
  }
  .vslam-slider .vslam-btn:hover:not(:disabled) { background: var(--vslam-blue-dark); }
  ...
  .vslam-slider .vslam-dot:hover { background: var(--vslam-blue); }
  .vslam-slider .vslam-dot.active { background: var(--vslam-blue); }
...
```{=html}
<div style="text-align: center; margin: 20px 0;">
  <img src="images/feature_extraction/bildpyramide.png" style="width: 100%; max-height: 500px; object-fit: contain;">
</div>
```
:::
...
::: {.vslam-slide}
#### Konzept: Was ist Visuelle Odometrie?

Die 40–500 Keypoints aus 1.1 sind nur **Punkte im Bild**, noch keine Bewegung. Visuelle Odometrie (VO) berechnet daraus, **wie sich die Kamera bewegt hat**.

**Grundprinzip:**
1. Frame $k$: Features erkannt an Position $(x, y)$
2. Frame $k+1$: Dieselben Features an neuer Position $(x', y')$
3. Aus der Verschiebung → Kamerabewegung schätzen

**VO-Pipeline in 4 Schritten:**
1. **Orientierung θ**: jeder Keypoint bekommt eine Richtung
2. **Binärer Deskriptor**: Fingerabdruck aus 256 Bits
3. **Matching**: Deskriptoren zwischen Frames vergleichen
4. **Pose-Schätzung**: Rotation R und Translation t berechnen

**Resultat:** Position und Orientierung der Kamera, Frame für Frame.
:::

::: {.vslam-slide}
#### Schritt 1: Orientierung θ (Rotationsinvarianz)

Wenn die Kamera sich dreht, sieht dieselbe Ecke aus einem anderen Winkel aus. Ohne Orientierung würde das Matching fehlschlagen.

**Lösung:** Jede Ecke bekommt eine Orientierung θ, die Richtung zum **Helligkeits-Schwerpunkt** im 31×31 Patch.

**Berechnung über Momente:**
$m_{00} = \sum I, \quad m_{10} = \sum x \cdot I, \quad m_{01} = \sum y \cdot I$
$\theta = \text{atan2}(m_{01},\, m_{10})$

**Konkretes Beispiel (3×3 Patch):**
- Helle Pixel rechts-unten: $m_{10} = +160$, $m_{01} = +80$
- $\theta = \text{atan2}(80, 160) \approx 27°$ → Pfeil zeigt zum Hellen

Dieselbe Ecke ergibt **immer denselben θ**, egal wie die Kamera gedreht ist.

```{=html}
<div style="text-align: center; margin: 16px 0;">
  <img src="images/feature_extraction/vo_orientierung_theta.png" style="width: 100%; max-height: 420px; object-fit: contain; border-radius: 6px;">
</div>
```
:::

::: {.vslam-slide}
#### Schritt 2a: Binärer Deskriptor (Prinzip)

Jede Ecke braucht einen **Fingerabdruck**, eine kompakte Beschreibung, die Matching ermöglicht.

**BRIEF-Deskriptor: 256 Pixelpaar-Vergleiche:**

Wähle 256 feste Pixelpaare $(A_i, B_i)$ im 31×31 Patch. Für jedes Paar:
$\text{Bit}_i = \begin{cases} 1 & \text{wenn } I(A_i) > I(B_i) \\ 0 & \text{sonst} \end{cases}$

Ergebnis: ein **256-Bit-String**, der Fingerabdruck der Ecke.

**Vorteile:**
- Extrem schnell zu berechnen (nur Vergleiche, keine Mathematik)
- Kompakt (256 Bit = 32 Bytes pro Feature)
- Einfach zu vergleichen (Hamming-Distanz via XOR)

```{=html}
<div style="text-align: center; margin: 16px 0;">
  <img src="images/feature_extraction/vo_deskriptor_prinzip.png" style="width: 100%; max-height: 480px; object-fit: contain; border-radius: 6px;">
</div>
```
:::

::: {.vslam-slide}
#### Schritt 2b: Binärer Deskriptor: Rotationsinvarianz (steered BRIEF)

**Problem:** Wenn die Kamera sich dreht, lesen die festen Paare andere Pixel ab → anderer Bitstring → kein Match.

**Lösung (steered BRIEF):** Die Pixelpaare werden um den Winkel **θ gedreht**, bevor sie abgelesen werden.

$\begin{pmatrix} x' \\ y' \end{pmatrix} = \begin{pmatrix} \cos\theta & -\sin\theta \\ \sin\theta & \cos\theta \end{pmatrix} \begin{pmatrix} x \\ y \end{pmatrix}$

- **Ohne Rotation:** Paare in fester Position → verschiedene Bits bei gedrehter Kamera
- **Mit Rotation um θ:** Paare drehen sich mit → **gleiche Bits** trotz Kameradrehung

Dasselbe Feature ergibt **immer denselben 256-Bit-String**, rotationsinvariant!

```{=html}
<div style="text-align: center; margin: 16px 0;">
  <img src="images/feature_extraction/vo_deskriptor_rotation.png" style="width: 100%; max-height: 440px; object-fit: contain; border-radius: 6px;">
</div>
```
:::

::: {.vslam-slide}
#### Schritt 3a: Feature Matching (Hamming-Distanz)

**Ziel:** Finde dieselbe Ecke in zwei aufeinanderfolgenden Frames.

**Methode (Hamming-Distanz):** Vergleiche zwei 256-Bit-Strings per XOR. Zähle unterschiedliche Bits.

$d_H(A, B) = \text{popcount}(A \oplus B)$

**Interpretation:**
- **Wenige unterschiedliche Bits** (z.B. 8/256) → wahrscheinlich **dieselbe Ecke** ✓
- **Viele unterschiedliche Bits** (z.B. 107/256) → **andere Ecke** ✗

**Konkret:** Gleiche Ecke: Hamming = 8 (3,1% Unterschied). Andere Ecke: Hamming = 107 (41,8% Unterschied). Der Unterschied ist deutlich!

```{=html}
<div style="text-align: center; margin: 16px 0;">
  <img src="images/feature_extraction/vo_hamming_distanz.png" style="width: 100%; max-height: 480px; object-fit: contain; border-radius: 6px;">
</div>
```
:::

::: {.vslam-slide}
#### Schritt 3b: Feature Matching (Lowe Ratio-Test)

**Problem:** Ähnliche Stellen im Bild (z.B. gleiche Fenstermuster) passen mehrfach → falsche Matches.

**Lösung (Lowe Ratio-Test):**
1. Finde die **zwei besten Matches** für jede Ecke (Hamming-Distanz $d_1 < d_2$)
2. Behalte nur wenn: $\dfrac{d_1}{d_2} < 0{,}75$

**Bedeutung:** Der beste Match muss **deutlich besser** sein als der zweitbeste.

**Konkrete Beispiele:**
- $d_1 = 12$, $d_2 = 80$ → $12/80 = 0{,}15 < 0{,}75$ → **behalten** ✓ (eindeutig)
- $d_1 = 40$, $d_2 = 45$ → $40/45 = 0{,}89 > 0{,}75$ → **verwerfen** ✗ (zu ähnlich)

**Resultat:** Nur grüne Matches übrig, zuverlässige, eindeutige Korrespondenzen.

```{=html}
<div style="text-align: center; margin: 16px 0;">
  <img src="images/feature_extraction/vo_feature_matching.png" style="width: 100%; max-height: 300px; object-fit: contain; border-radius: 6px;">
</div>
<p style="text-align:center; color:#aaa; font-size:0.85em; margin-top:6px;">377 gute Matches zwischen zwei Frames, grüne Linien verbinden dieselbe Ecke</p>
```
:::

::: {.vslam-slide}
#### Schritt 4a: Pose-Schätzung (Essential Matrix)

Aus den gematchten Punktpaaren $(x, x')$ berechnen wir **Rotation R** und **Translation t** der Kamera.

**Epipolar-Bedingung:** Für jedes korrekte Punktpaar gilt:
$x'^T \cdot E \cdot x = 0$

**Essential Matrix E** kodiert Rotation und Translation:
$E = [t]_\times \cdot R$

Wo $[t]_\times$ die schiefsymmetrische Matrix von $t$ ist.

**Pipeline:**
1. `findEssentialMat` mit **RANSAC**: berechnet E aus mind. 5 Paaren, markiert Outlier
2. `recoverPose` zerlegt E per **SVD** in R und t
3. **Einschränkung:** t hat nur Richtung; bei Mono-Kamera bleibt die echte Strecke unbekannt

RANSAC = Random Sample Consensus: probiert zufällige Teilmengen, wählt das Modell mit den meisten Inliern.
:::

::: {.vslam-slide}
#### Schritt 4b: Pose-Schätzung (Inlier & Outlier)

RANSAC unterscheidet:
- **Inlier:** Punkte, die zur geschätzten Bewegung passen ($x'^T E x \approx 0$)
- **Outlier:** Punkte, die nicht passen → verworfen

**Konkretes Ergebnis aus echten Frames:**

```{=html}
<div style="text-align: center; margin: 16px 0;">
  <img src="images/feature_extraction/vo_pose_inlier_outlier.png" style="width: 100%; max-height: 480px; object-fit: contain; border-radius: 6px;">
</div>
```

- **Grün** = Inlier (44 Punkte): passen zur Bewegung ✓
- **Rot** = Outlier (14 Punkte): verworfen ✗
- Translation zeigt nach **vorne** (Kamera fährt vorwärts)
- Rotation = fast reines **Gieren** (leichte Rechtsdrehung)
:::

::: {.vslam-slide}
#### Schritt 5: Trajektorie (Akkumulation der Bewegung)

Jedes Frame-Paar liefert nur die **relative** Bewegung ($R$, $t$). Die **Trajektorie** entsteht durch Aufsummieren.

**Algorithmus:**
$P_{\text{neu}} = P_{\text{alt}} + R_{\text{global}} \cdot t$
$R_{\text{global}} = R_{\text{global}} \cdot R$

- $R_{\text{global}}$ dreht das lokale „vorne“ in die **Weltrichtung**
- Start: $P_0 = (0,0,0)$, $R_{\text{global}} = I$

**Konkretes 2D-Beispiel (t immer (1,0)):**

| Schritt | θ | $R_{\text{global}} \cdot t$ | Neue Position |
|:---:|:---:|:---:|:---:|
| 1 | 0° | $(+1, 0)$ | $(1, 0)$ |
| 2 | 90° | $(0, +1)$ | $(1, 1)$ |
| 3 | 180° | $(-1, 0)$ | $(0, 1)$ |

Lokales „vorne“ bleibt gleich, aber die Weltrichtung dreht sich!

```{=html}
<div style="text-align: center; margin: 16px 0;">
  <img src="images/feature_extraction/vo_trajektorie.png" style="width: 100%; max-height: 440px; object-fit: contain; border-radius: 6px;">
</div>
```
:::

::: {.vslam-slide}
#### Zusammenfassung 1.2 & Ausblick

**1.2 Visuelle Odometrie (vollständige Pipeline):**

1. **θ (Orientierung):** Helligkeits-Schwerpunkt → rotationsinvariante Richtung
2. **Deskriptor:** 256 Pixelpaar-Vergleiche → Bit-String (steered BRIEF)
3. **Matching:** Hamming-Distanz + Ratio-Test → zuverlässige Korrespondenzen
4. **Essential Matrix E:** Epipolar-Bedingung + RANSAC → Inlier filtern
5. **Pose (R, t):** SVD-Zerlegung → Rotation und Translation
6. **Trajektorie:** $P = P + R_{\text{global}} \cdot t$ → Kameraweg

**Verbleibende Probleme:**
- **Drift:** Kleine Fehler summieren sich → Trajektorie weicht ab
- **Nur 2D:** Punkte haben noch keine echte Tiefe (3D-Koordinaten fehlen)
- **Skalierung:** Mono-Kamera kennt keine absoluten Abstände

→ Diese Probleme lösen **2.1 Lokales Mapping** (3D-Rekonstruktion) und **3.1/3.2 Loop Closure** (Drift-Korrektur).

```{=html}
<div style="text-align: center; margin: 16px 0;">
  <img src="images/vc_anlage_2.gif" width="60%" style="border-radius: 6px;">
  <p style="color:#aaa; font-size:0.85em; margin-top:6px;">Kamerabewegung mit visueller Odometrie in Echtzeit</p>
</div>
```
:::
...
<button class="vslam-btn vslam-prev">← Zurück</button>
<div class="vslam-dots"></div>
<span class="vslam-counter">Slide 1 / 10</span>
<button class="vslam-btn vslam-next">Weiter →</button>
:::
...
::: {.vslam-slide}
#### Schritt 3a: Pose-Schätzung: Perspective-n-Point (PnP)

Für Kamera 0 und 1 liefert die **Essential Matrix** (Step 1.2) die erste Relativpose, rein aus 2D-2D-Korrespondenzen, ganz ohne 3D-Wissen. Sobald aus diesem ersten Paar aber 3D-Landmarks trianguliert sind, steht für alle weiteren Keyframes eine stärkere Informationsquelle zur Verfügung: **bekannte 3D-Punkte + ihre 2D-Beobachtung** im neuen Bild.

**PnP-Problem:** Gegeben $n$ Korrespondenzen zwischen 3D-Punkten $X_j$ und ihren 2D-Projektionen $x_j$ im Kamerabild sowie die bekannte Kalibrierung $K$; gesucht sind $R$ und $t$, sodass

$$
x_j \;\sim\; K\,(R\,X_j + t) \quad \text{für alle } j
$$

**Minimale Lösung:** 3 Korrespondenzen reichen theoretisch (**P3P**), üblich ist eine lineare Lösung ab 4 Punkten (**EPnP**) mit anschließender nichtlinearer Verfeinerung.

**Im Notebook:** `cv2.solvePnPRansac`: RANSAC über zufällige Minimal-Stichproben verwirft Ausreißer-Tracks, danach iterative Verfeinerung über alle Inlier. Kamera $k$ wird so direkt gegen die bereits vorhandenen Landmarks aus Kamera 0/1 registriert, unabhängig von den übrigen Keyframes.
:::

::: {.vslam-slide}
#### Schritt 3b: PnP vs. Essential Matrix: kurzer Vergleich

Beide Verfahren schätzen $(R, t)$ einer Kamera, unterscheiden sich aber in Eingabedaten und Einsatzpunkt:

| | **Essential Matrix** | **PnP** |
|---|---|---|
| Eingabe | 2D ↔ 2D (zwei Bilder) | 2D ↔ 3D (Bild + Landmark) |
| Mindestpunkte | 5 (Nister-Verfahren) | 3 (P3P) / 4 (EPnP) |
| Benötigt 3D-Punkte? | Nein | Ja |
| Maßstab | unbekannt (nur Richtung von $t$) | geerbt von den 3D-Punkten, konsistent zur fixierten Skala |
| Einsatz in der Pipeline | einmalig, erstes Keyframe-Paar (Kamera 0 → 1) | jedes weitere Keyframe (Kamera 2, 3, …) |

**Kernidee:** Die Essential Matrix „bootstrapt“ die Karte, ohne dass 3D-Punkte existieren. Sobald ein erstes 3D-Grundgerüst steht, ist PnP das direktere Werkzeug: weniger Mindestkorrespondenzen, kein erneuter Skalenverlust, und pro Kamera unabhängig lösbar.
:::

::: {.vslam-slide}
#### Schritt 4: Mathematik: Triangulation (DLT)
...
::: {.vslam-slide}
#### Schritt 5: Beispiel: lokale Karte aus 3 Keyframes unseres Datensatzes
...
- **Mono-Skalenmehrdeutigkeit**: Eine einzelne Kamera kann **Maßstab** nicht beobachten. Wir fixieren ihn willkürlich (z.B. $\|t_1\| = 1$); da PnP die Skala der bestehenden Landmarks erbt, bleiben alle weiteren Posen automatisch konsistent dazu.
...
::: {.vslam-slide}
#### Schritt 1: Reprojektionsfehler: der Optimierungs-Anker
...
::: {.vslam-slide}
#### Schritt 2: Konvisibilitätsgraph
...
::: {.vslam-slide}
#### Schritt 3: Mathematik: Bundle Adjustment
...
::: {.vslam-slide}
#### Schritt 4: Robustheits-Pipeline
...
::: {.vslam-slide}
#### Schritt 5: Beispiel: Bundle Adjustment auf unseren Keyframes
...
::: {.vslam-slide}
#### Das Drift-Problem & Loop-Closure-Prinzip
...
**Loop Closure** erkennt, wann die Kamera an einen bereits besuchten Ort
zurückkehrt: Zwei KeyFrames, die **nah im Raum** aber **weit in der Zeit**
auseinanderliegen, zeigen denselben Ort, ein **Loop**.
...
<div style="text-align: center; margin: 8px 0;">
  <img src="images/loop_closure/distmatrix.png" style="max-width: 100%; border-radius: 6px;">
</div>
```

Aus ~349 Frames → **236 rohe Paare** → **Non-Maximum Suppression** reduziert
auf **84 Kandidaten** (einer pro zeitlichem Cluster). Der Filter ist bewusst
liberal, lieber zu viele als echte Loops zu verpassen; das teure BoW- und
Geometrie-Scoring trennt anschließend die echten Loops heraus.
:::
...
```{=html}
<div style="text-align: center; margin: 8px 0;">
  <img src="images/loop_closure/bow_similarity.png" style="max-width: 100%; border-radius: 6px;">
</div>
```

Die hellen Diagonalstreifen zeigen Frames mit hoher visueller Ähnlichkeit;
genau dort liegen potenzielle Loops.
:::
...
```{=html}
<div style="text-align: center; margin: 8px 0;">
  <img src="images/loop_closure/trajectory_loops.png" style="max-width: 100%; border-radius: 6px;">
</div>
```

Die roten Linien zeigen die Top-8 Kandidaten; alle liegen an Stellen,
an denen die Kamera dieselbe Parkhausrunde ein zweites Mal durchfährt.
BoW allein kann falsch-positive liefern (ähnliche Struktur an anderem Ort);
die räumliche Nähe eliminiert diese.
:::
...
|------|------|-------|---------|--------|
| #1 | KF255↔KF294 | 40% | 178 | ✓ verifiziert |
| #2 | KF244↔KF284 | 51% | 267 | ✓ verifiziert |
| #8 | KF166↔KF198 | **53%** | 278 | ✓ verifiziert |

Alle **8 von 8** Kandidaten bestehen die geometrische Prüfung (Ratios 28–53 %).
Das zeigt: Die Vorfilterung aus räumlicher Nähe **und** BoW-Ähnlichkeit ist bereits
so selektiv, dass kaum Falsch-Positive durchkommen; die geometrische Verifikation
**bestätigt** die Loops und ist die letzte Absicherung gegen verzerrende Kanten.
:::

::: {.vslam-slide}
#### Ergebnis: KF255 ↔ KF294 → Loop-Constraint
Der am höchsten bewertete Loop (#1): **195 Original-Frames (21.6 s)** zwischen den
Aufnahmen, die Kamera kehrt auf **0.06 m** genau an dieselbe Position zurück.

```{=html}
<div style="display: flex; gap: 16px; justify-content: center; align-items: start; margin: 8px 0;">
  <div style="text-align: center;">
    <img src="images/loop_frames/frame_kf255.png" style="width: 100%; max-width: 350px;">
    <div style="font-size: 0.8em; color: #aaa; margin-top: 2px;">KF 255: frühere Aufnahme</div>
  </div>
  <div style="text-align: center;">
    <img src="images/loop_frames/frame_kf294.png" style="width: 100%; max-width: 350px;">
    <div style="font-size: 0.8em; color: #aaa; margin-top: 2px;">KF 294: spätere Aufnahme</div>
  </div>
</div>
```
...
| Metrik | Wert |
|--------|------|
| Räumliche Distanz | 0.06 m |
| 3D-Überlappung | 40% (178 Punkte) |
| Zeitlicher Abstand | 195 Frames × 0.111 s = 21.6 s |

Insgesamt bestehen **8 Loop-Paare** die Verifikation. Sie werden als
`loop_constraints.json` gespeichert und fließen als Loop-Closure-Kanten
in die Pose-Graph-Optimierung (3.2) ein.
:::
...
::: {.vslam-slide}
#### Schritt 2: Fehler auf der Lie-Algebra $\mathfrak{se}(3)$
...
::: {.vslam-slide}
#### Schritt 3: Das Optimierungsproblem
...
::: {.vslam-slide}
#### Schritt 5: Sparse Lineare Least-Squares-Lösung
...
::: {.vslam-slide}
#### Schritt 6: Ergebnis Teil 1: Trajektorie vorher/nachher
...
::: {.vslam-slide}
#### Schritt 7: Ergebnis Teil 2: Fehleranalyse
...
"""
Feature Extraction — wiederverwendbare Implementierung (Allgemeinfall)

Funktioniert mit jedem beliebigen Bild (Farbe oder Graustufen).

Pipeline (Kapitel 1.1):
  1. Graustufen + Gauss-Filter (entrauschtes 1-Kanal-Bild)
  2. Bildpyramide (8 Stufen, Faktor 1.2)
  3. FAST-Detektor auf allen Stufen
  4. Rückrechnung in Original-Koordinaten (× 1.2^k)
  5. Quadtree-Verteilung (stärkster Keypoint pro Zelle)
  6. Harris Corner Scoring → Top-N stabilste Keypoints
"""
...
def apply_quadtree(kps, w, h, grid_nx=15, grid_ny=8):
    ...
    pro Gitterzelle bleibt nur der stärkste Keypoint."""
    cw, ch = max(1, w // grid_nx), max(1, h // grid_ny)
    qt_features = []
...
def extrahiere_features(bild, n_stufen=8, skala=1.2, fast_threshold=15,
                        grid_nx=15, grid_ny=8, n_top=40, verbose=True):
    """
    Extrahiert robuste, gleichmäßig verteilte Keypoints aus einem
    beliebigen Bild.

    Parameter:
      bild           beliebiges Bild (BGR-Farbbild oder Graustufen)
      n_stufen       Anzahl Pyramidenstufen
      skala          Skalierungsfaktor der Pyramide
      fast_threshold Schwellenwert t des FAST-Detektors
      grid_nx/ny     Quadtree-Gitter (wird an kleine Bilder angepasst)
      n_top          finale Anzahl Keypoints (Top-Harris)

    Rückgabe: Liste von cv2.KeyPoint in Original-Koordinaten
    """
    # ── Schritt 1: Graustufen + Gauss-Filter ──
    # Farbbild? → I = 0.299·R + 0.587·G + 0.114·B (macht cv2 intern)
    if bild.ndim == 3:
        grau = cv2.cvtColor(bild, cv2.COLOR_BGR2GRAY)
    else:
        grau = bild.copy()

    # 5×5 Gauss-Kernel, σ=1: glättet Rauschen, erhält echte Kanten
    ...
    # ── Schritt 2+3: Bildpyramide + FAST auf allen Stufen ──
    fast = cv2.FastFeatureDetector_create(threshold=fast_threshold,
                                          nonmaxSuppression=True)

    alle_kandidaten = []   # Keypoints in Original-Koordinaten
    bild_akt = geglättet.copy()

    for stufe in range(n_stufen):
        h, w = bild_akt.shape
        if min(h, w) < 32:   # Stufe zu klein → Pyramide beenden
            break

        kps = fast.detect(bild_akt, None)

        # Schritt 4: Rückrechnung in Original-Koordinaten (× skala^k)
        faktor = skala ** stufe
        for k in kps:
            alle_kandidaten.append(cv2.KeyPoint(
                x=k.pt[0] * faktor,
                y=k.pt[1] * faktor,
                size=k.size * faktor,
                response=k.response,
        ...
        if verbose:
            print(f"Stufe {stufe}: {w:4d}×{h:3d} px → {len(kps):4d} FAST-Ecken")

        bild_akt = cv2.resize(bild_akt, (int(w / skala), int(h / skala)))

    if verbose:
        print(f"\nFAST-Kandidaten gesamt (alle Stufen): {len(alle_kandidaten)}")

    # ── Schritt 5: Quadtree-Verteilung ──
    # Gitter an Bildgröße anpassen, damit auch kleine Bilder funktionieren
    nx = min(grid_nx, max(1, W // 30))
    ny = min(grid_ny, max(1, H // 30))
    qt_f = apply_quadtree(alle_kandidaten, W, H, nx, ny)
    if verbose:
        print(f"Nach Quadtree ({nx}×{ny}-Gitter, 1 pro Zelle): {len(qt_f)}")

    # ── Schritt 6: Harris Corner Scoring ──
    # R = det(M) − k·trace(M)², k ≈ 0.04
    harris = cv2.cornerHarris(geglättet.astype(np.float32),
                              blockSize=3, ksize=3, k=0.04)

    kps_hs = [(k, float(harris[int(k.pt[1]), int(k.pt[0])]))
              for k in qt_f
              if 0 <= int(k.pt[1]) < harris.shape[0]
              and 0 <= int(k.pt[0]) < harris.shape[1]]
    kps_hs.sort(key=lambda x: -x[1])

    n_behalten = min(n_top, len(kps_hs))
    keypoints_final = [k for k, s in kps_hs[:n_behalten]]

    # ── Ergebnis: der Feature-Extraction-Trichter ──
    if verbose:
        print(f"Nach Top-Harris: {n_behalten}")
        print(f"\nTrichter: {len(alle_kandidaten)} FAST → "
              f"{len(qt_f)} Quadtree → {n_behalten} Top-Harris")
        print("\nFinale Keypoints (x, y, Stufe, Harris-Score):")
        for (k, score) in kps_hs[:10]:
            print(f"  ({k.pt[0]:6.1f}, {k.pt[1]:6.1f})  "
                  f"Stufe {k.octave}  R = {score:+.4f}")
        print("  ...")
...
# ──────────────────────────────────────────────
# Beispielaufruf — hier eigenes Bild einsetzen
# ──────────────────────────────────────────────
if __name__ == "__main__":
    bild = cv2.imread("mein_bild.png")   # <-- eigener Bildpfad
    if bild is None:
        raise FileNotFoundError("Bild konnte nicht geladen werden — "
                                "Pfad prüfen!")

    keypoints = extrahiere_features(bild)
    print(f"\n{len(keypoints)} finale Keypoints extrahiert.")
```
...
"""
Visuelle Odometrie — wiederverwendbare Implementierung (Allgemeinfall)

Funktioniert mit jeder beliebigen Bildfolge (≥ 2 Frames,
Farbe oder Graustufen).

Pipeline (Kapitel 1.2):
  1. ORB-Features: Orientierung θ (Intensity Centroid)
     + steered BRIEF (256-Bit-Deskriptor)
  2. Matching: Hamming-Distanz + Lowe's Ratio-Test
  3. Essential Matrix (RANSAC) + recoverPose → R, t
  4. Trajektorie: Akkumulation der Relativposen
"""
...
def berechne_trajektorie(frames, K=None, nfeatures=500, ratio=0.75,
                         verbose=True):
    """
    Berechnet die Kamera-Trajektorie aus einer beliebigen Bildfolge.

    Parameter:
      frames     Liste von Bildern (BGR oder Graustufen), zeitlich sortiert
      K          Kamera-Matrix 3×3; None → generische Schätzung
                 (echte Kalibrierung liefert deutlich genauere Ergebnisse!)
      nfeatures  Anzahl ORB-Features pro Frame
      ratio      Schwellenwert des Lowe-Ratio-Tests

    Rückgabe: Liste der Kamera-Positionen (np.array mit x, y, z)
    """
    if len(frames) < 2:
        raise ValueError("Mindestens 2 Frames nötig!")

    # Farbbilder → Graustufen
    frames_gray = [cv2.cvtColor(f, cv2.COLOR_BGR2GRAY) if f.ndim == 3 else f
                   for f in frames]

    # ── Kamera-Parameter ──
    h0, w0 = frames_gray[0].shape
    if K is None:
        # Generische Schätzung: Brennweite ≈ Bilddiagonale,
        # Hauptpunkt = Bildmitte
        fx = fy = float(max(w0, h0))
        cx, cy = w0 / 2, h0 / 2
    else:
        fx, fy = K[0, 0], K[1, 1]
        cx, cy = K[0, 2], K[1, 2]

    # ── Schritt 1+2: ORB (θ + steered BRIEF) und Matcher ──
    # ORB berechnet intern:
    #   θ = atan2(m01, m10)  (Helligkeits-Schwerpunkt im 31×31 Patch)
    #   Deskriptor: 256 um θ gedrehte Pixelpaar-Vergleiche → 256 Bit
    orb = cv2.ORB_create(nfeatures=nfeatures)
    bf  = cv2.BFMatcher(cv2.NORM_HAMMING, crossCheck=False)

    # Speichere alle R und t Matrizen der Frame-Paare
    poses = []

    for pair_idx in range(len(frames_gray) - 1):
        frame_a = frames_gray[pair_idx]
        frame_b = frames_gray[pair_idx + 1]

        # ORB Features (Position, θ, Deskriptor)
        kp_a, des_a = orb.detectAndCompute(frame_a, None)
        kp_b, des_b = orb.detectAndCompute(frame_b, None)

        if des_a is None or des_b is None:
            if verbose:
                print(f"Paar {pair_idx} → {pair_idx+1}: "
                      f"keine Features — übersprungen")
            continue

        # ── Schritt 3: Matching (Hamming) + Lowe's Ratio-Test ──
        matches_raw = bf.knnMatch(des_a, des_b, k=2)
        good_m = []
        ...
        if len(good_m) < 5:
            if verbose:
                print(f"Paar {pair_idx} → {pair_idx+1}: nur "
                      f"{len(good_m)} Matches (< 5) — übersprungen")
            continue

        pts_a = np.float32([kp_a[m.queryIdx].pt for m in good_m])
        pts_b = np.float32([kp_b[m.trainIdx].pt for m in good_m])

        # ── Schritt 4: Essential Matrix (RANSAC) + Pose ──
        ...
        if E is None:
            if verbose:
                print(f"Paar {pair_idx} → {pair_idx+1}: "
                      f"keine Essential Matrix — übersprungen")
            continue

        # SVD-Zerlegung von E → R, t (t nur als Richtung, Mono-Skala!)
        n_inlier, R, t, mask = cv2.recoverPose(E, pts_a, pts_b,
                                               focal=fx, pp=(cx, cy))
        poses.append((R, t))

        if verbose:
            print(f"Paar {pair_idx} → {pair_idx+1}: "
                  f"{len(good_m):3d} gute Matches, {n_inlier:3d} Inlier")

    # ── Schritt 5: Trajektorie akkumulieren ──
    ...
    if verbose:
        ...
        print(f"\nKamera-Trajektorie über {len(positions)} Frames:")
        for i, pos in enumerate(positions):
            print(f"  Frame {i}: Position "
                  f"({pos[0]:+.3f}, {pos[1]:+.3f}, {pos[2]:+.3f})")
        print(f"\nGesamtbahn: {total_distance:.3f} Einheiten "
              f"(Mono: Skala unbekannt)")
        print("Fehler akkumuliert sich über Zeit → das Drift-Problem!")
        print("→ Lösung: Lokales Mapping/BA (2.x) und Loop Closure (3.x)")

    return positions


# ──────────────────────────────────────────────
# Beispielaufruf — hier eigenen Bildordner einsetzen
# ──────────────────────────────────────────────
if __name__ == "__main__":
    BILD_ORDNER = "meine_bilder"   # <-- eigener Ordner mit Bildfolge

    bildpfade = sorted(glob.glob(os.path.join(BILD_ORDNER, "*.png"))
                       + glob.glob(os.path.join(BILD_ORDNER, "*.jpg")))
    if len(bildpfade) < 2:
        raise FileNotFoundError("Mindestens 2 Bilder im Ordner nötig — "
                                "Pfad prüfen!")

    frames = [cv2.imread(p) for p in bildpfade]
    print(f"{len(frames)} Frames geladen aus '{BILD_ORDNER}'")

    # Optional: eigene Kamera-Matrix übergeben, z.B.
    # K = np.array([[fx, 0, cx], [0, fy, cy], [0, 0, 1]])
    trajektorie = berechne_trajektorie(frames, K=None)
```
