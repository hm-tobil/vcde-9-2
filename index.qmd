...
### Feature Extraction
<details markdown="1">
<summary><b>Vollständige Implementierung anzeigen (11_feature_extraction.qmd)</b></summary>

```python

"""
Feature Extraction — Logik (ohne Plots)
...
# ──────────────────────────────────────────────
BILD_ORDNER    = "../cam0"
SKALA          = 1.2    # Skalierungsfaktor der Pyramide
N_STUFEN       = 8      # Pyramidenstufen
FAST_THRESHOLD = 15     # Schwellenwert t des FAST-Detektors
GRID_NX, GRID_NY = 15, 8  # Quadtree-Gitter
N_TOP          = 40     # finale Anzahl Keypoints (Top-Harris)

# ──────────────────────────────────────────────
# Schritt 1: Graustufen + Gauss-Filter
# ──────────────────────────────────────────────
bildpfade = sorted(glob.glob(f"{BILD_ORDNER}/*.png"))
bild_bgr  = cv2.imread(bildpfade[0])

# RGB → Graustufen: I = 0.299·R + 0.587·G + 0.114·B (macht cv2 intern)
...
for stufe in range(N_STUFEN):
    ...
    # Schritt 4: Rückrechnung in Original-Koordinaten (× 1.2^k)
    faktor = SKALA ** stufe
    for k in kps:
...
qt_f = apply_quadtree(alle_kandidaten, W, H, GRID_NX, GRID_NY)
print(f"Nach Quadtree (1 pro Zelle):          {len(qt_f)}")
...
### Visuelle Odometrie
<details markdown="1">
<summary><b>Vollständige Implementierung anzeigen (12_visuelle_odometrie.qmd)</b></summary>

```python

"""
Visuelle Odometrie — Logik (ohne Plots)
...
# ──────────────────────────────────────────────
BILD_ORDNER = "../cam0"
STRIDE      = 25   # Abstand zwischen den Demo-Frames
N_FRAMES    = 6    # → 5 konsekutive Frame-Paare

# ──────────────────────────────────────────────
# Frames laden
# ──────────────────────────────────────────────
bildpfade   = sorted(glob.glob(f"{BILD_ORDNER}/*.png"))
demo_pfade  = bildpfade[::STRIDE][:N_FRAMES]
frames_gray = [cv2.cvtColor(cv2.imread(p), cv2.COLOR_BGR2GRAY)
               for p in demo_pfade]
print(f"{len(frames_gray)} Frames geladen "
      f"(jeder {STRIDE}. aus {BILD_ORDNER})")
...
for pair_idx in range(N_FRAMES - 1):
...
print(f"\nGesamtbahn: {total_distance:.3f} Einheiten (Mono: Skala unbekannt)")
